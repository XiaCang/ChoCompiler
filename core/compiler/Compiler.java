...
public class Compiler {
    ...
    public CompiledProgram compile(ASTNode ast) {
        ...
        return new CompiledProgram(curInstructions(), consts);
    }
    ...
    private void genAssign(AssignExpression ae) {
        ...
        if (ae.getLeft().type() == ASTNodeType.IndexExpression) {
            ...
            gen(Opcode.SetLocal, st.get(id.getValue()).index());
            gen(Opcode.GetLocal, st.get(id.getValue()).index());
        }
    ...
    private void genCall(CallExpression ce) {
        ...
        for (int i = ce.getArguments().size() - 1; i >= 0; i--) {
